<template>
  <div class="section-container py-8 md:py-16" :class="containerClasses" ref="sectionRef">
    <div class="default-grid !gap-y-12">
      <div class="col-span-full" v-if="data.eyebrow || data.header || data.subheader" ref="headlineRef">
        <HeadlineFields :data="data" :index="index"></HeadlineFields>
      </div>

      <div class="col-span-full accordion-container" ref="accordionContainer">
        <AccordionItem
            v-for="(item, index) in data.accordion_items"
            :key="index"
            :index="index"
            :item="item"
...
onMounted(() => {
  ...
  // Get all accordion items and animate them with a stagger
  const accordionItems = Object.values(accordionItemRefs.value);
  if (accordionItems.length > 0) {
    const elements = accordionItems.map(item => item.$el);
    ...
    timeline.add(
      $gsap.fromTo(
        ...
        { 
          ...
          ease: "power2.out", 
          stagger: 0.1,
          clearProps: "all" // Clear props after animation to avoid conflicts with accordion functionality
        }
      ),
      "-=0.4"
    );
